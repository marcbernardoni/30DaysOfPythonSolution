...
for i in range(len(data)):
    for key in data[i]:
        if key == 'languages':
            languages.append(data[i][key])

flat_languages = []
for sublist in languages:
    for item in sublist:
        flat_languages.append(item)

flat_languages = list(dict.fromkeys(flat_languages))
flat_languages.sort()
print(f"the total number of languages in the data is {len(flat_languages)}")

# b - Find the ten most spoken languages from the data
lang_s = {}

for lang in range(len(flat_languages)):
    lang_current = flat_languages[lang]
    for i in range(len(data)):
        pop = 0
        for key in data[i]:
            if key == 'languages' and lang_current in data[i][key]:
                pop += data[i]['population']
                # print(pop)
                # print(f'La langue {lang_current} est parlée par {pop} personnes')
...
for i in range(len(data)):
    for key, value in data[i].items():
        country[data[i].get('name')] = data[i].get('population')
